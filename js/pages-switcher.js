...
"use strict";
    ...
    function init(options) {
        ...
        window.onhashchange = function(event) {
            if(location.hash) {
                ...
                var menuLink = $(menu+' a[href*="'+location.hash.split('index.html')[0]+'"]');
    ...
    function ajaxLoader() {
        ...
        var href = $('.ajax-page-load').each(function(){
            href = $(this).attr('href');
            if(location.hash == location.hash.split('index.html')[0] + '/' + href.substr(0,href.length-5)){
        ...
        $(document)
            .on("click",".site-main-menu, #ajax-page-close-button", function (e) { // Hide Ajax Loaded Page on Navigation cleck and Close button
                e.preventDefault();
                hideContent();
                location.hash = location.hash.split('index.html')[0];
            })
            .on("click",".ajax-page-load", function () { // Show Ajax Loaded Page
                var hash = location.hash.split('index.html')[0] + '/' + $(this).attr('href').substr(0,$(this).attr('href').length-5);
                location.hash = hash;
                showContent();
